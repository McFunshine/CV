% Professional CV Template
\documentclass[11pt,a4paper]{article}

% Required packages
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{geometry}
\usepackage{hyperref}
\usepackage{fontawesome}
\usepackage{titlesec}
\usepackage{enumitem}
\usepackage{xcolor}
\usepackage[english]{datetime2}
\usepackage{ragged2e}
\usepackage{needspace}


% Custom colors
\definecolor{primary}{RGB}{70,130,180}
\definecolor{secondary}{RGB}{80,80,80}

% Page geometry
\geometry{
    left=1.5cm,
    right=1.5cm,
    top=1.5cm,
    bottom=1.5cm
}

% Custom section style
\titleformat{\section}
    {\Large\bfseries\color{primary}}
    {}{0em}
    {}[\titlerule]
\titlespacing*{\section}{0pt}{12pt}{8pt}

% Add custom spacing for company entries
\setlength{\parindent}{0pt}
\newcommand{\company}[2]{\noindent\textbf{#1} \hfill \textit{#2}\\}

% Custom commands for contact info
\newcommand{\contactItem}[2]{
    {\color{secondary}\small#1} & #2 \\
}

% After the packages section, add this new command
\newenvironment{companylist}{%
    \begin{itemize}[leftmargin=0pt,label={}]
}{%
    \end{itemize}
}

% After the packages section, add these commands
\setlength{\parindent}{0pt}
\hyphenpenalty=10000  % Reduce hyphenation
\exhyphenpenalty=10000  % Reduce hyphenation
\widowpenalty=10000  % Prevent widow lines
\clubpenalty=10000   % Prevent club lines

% After the packages section, add:
\input{cv_texts.tex}

% Add this command definition before \begin{document}
\newcommand{\companyheader}[3]{%
  \noindent\begin{minipage}{\textwidth}%
    \textbf{#1} {\color{secondary}\small(#2)}\hfill\textit{#3}%
  \end{minipage}\\[2pt]%
}

% Document begins
\begin{document}

% Header
\begin{center}
    {\Huge\textbf{\MakeUppercase{\ProfileName}}}\\[0.5em]
    {\color{secondary}\large \CVSubtitleGeneral}\\[1em]
\end{center}

% Contact Information
\begin{tabular}{ll@{\hspace{0.2\textwidth}}ll}
    \faPhone & \ProfilePhone & \faLinkedin & \href{https://www.\ProfileLinkedIn}{\texttt{\ProfileLinkedIn}} \\
    \faEnvelope & \href{mailto:\ProfileEmail}{\texttt{\ProfileEmail}} & \faGlobe & \href{https://\ProfileWebsite}{\texttt{\ProfileWebsite}} \\
    \faMapMarker & \ProfileLocation & \faGithub & \href{https://\ProfileGithub}{\texttt{\ProfileGithub}} \\
    \faFlag & \ProfileCitizenship & & \\
\end{tabular}

\section{Summary}
\ProfileGeneral

\section{Experience}
\begin{itemize}[leftmargin=0pt,label={},itemsep=2em,topsep=0pt]

\item \parbox{\textwidth}{\textbf{\KlinkendetaalName} {\color{secondary}\small(\KlinkendetaalCompanyDescription)} \hfill \textit{\KlinkendetaalLocation}}\\
\textit{\KlinkendetaalJobTitle} \hfill \textit{\KlinkendetaalJobStart\,--\,\KlinkendetaalJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \KlinkendetaalTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \KlinkendetaalFineTuning
    \item \KlinkendetaalModelSelection
    \item \KlinkendetaalBenchmark
    \item \KlinkendetaalDataPipeline
    \item \KlinkendetaalRLHF
    \item \KlinkendetaalKotlinEval
    \item \KlinkendetaalLegacyJava
\end{itemize}


\item \parbox{\textwidth}{\textbf{\KhonraadName} {\color{secondary}\small(\KhonraadCompanyDescription)} \hfill \textit{\KhonraadLocation}}\\
\textit{\KhonraadThirdJobTitle} \hfill \textit{\KhonraadThirdJobStart\,--\,\KhonraadThirdJobEnd}\\
\textit{\KhonraadSecondJobTitle} \hfill \textit{\KhonraadSecondJobStart\,--\,\KhonraadSecondJobEnd}\\
\textit{\KhonraadFirstJobTitle} \hfill \textit{\KhonraadFirstJobStart\,--\,\KhonraadFirstJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \KhonraadTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \KhonraadJava
    \item \KhonraadInnovation
    \begin{itemize}[leftmargin=*,topsep=0pt,parsep=0pt,partopsep=0pt,itemsep=0pt,label={--}]
        \item \KhonraadChatbots
        \item \KhonraadMobile
        \item \KhonraadAndroid
        \item \KhonraadMPS
    \end{itemize}
\end{itemize} 


\item \parbox{\textwidth}{\textbf{\McfunshineName} {\color{secondary}\small(\McfunshineCompanyDescription)} \hfill \textit{\McfunshineLocation}}\\
\textit{\McfunshineJobTitle} \hfill \textit{\McfunshineJobStart\,--\,\McfunshineJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \McfunshinePodmorphTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \McfunshinePodmorph
    \item \McfunshinePodmorphAI
    \item \McfunshinePodmorphBaker
\end{itemize}

\item \parbox{\textwidth}{\textbf{\BakerName} {\color{secondary}\small(\BakerCompanyDescription)} \hfill \textit{\BakerLocation}}\\
\textit{\BakerThirdJobTitle} \hfill \textit{\BakerThirdJobStart\,--\,\BakerThirdJobEnd}\\
\textit{\BakerSecondJobTitle} \hfill \textit{\BakerSecondJobStart\,--\,\BakerSecondJobEnd}\\
\textit{\BakerFirstJobTitle} \hfill \textit{\BakerFirstJobStart\,--\,\BakerFirstJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \BakerTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \BakerEcosystem
    \item \BakerTraining
    \item \BakerConferences
    \item \BakerSupport
    \item \BakerSDK
\end{itemize}

\item \parbox{\textwidth}{\textbf{\KempenName} {\color{secondary}\small(\KempenCompanyDescription)} \hfill \textit{\KempenLocation}}\\
\textit{\KempenSecondJobTitle} \hfill \textit{\KempenSecondJobStart\,--\,\KempenSecondJobEnd}\\
\textit{\KempenFirstJobTitle} \hfill \textit{\KempenFirstJobStart\,--\,\KempenFirstJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \KempenTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \KempenTraining
    \item \KempenIndexing
    \item \KempenTrading
    \item \KempenWorkflow
\end{itemize}

\item \parbox{\textwidth}{\textbf{\UnisysName} {\color{secondary}\small(\UnisysCompanyDescription)} \hfill \textit{\UnisysLocation}}\\
\textit{\UnisysJobTitle} \hfill \textit{\UnisysJobStart\,--\,\UnisysJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \UnisysTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \UnisysReporting
    \item \UnisysDecisions
\end{itemize}

\item \parbox{\textwidth}{\textbf{\ACTName} {\color{secondary}\small(\ACTCompanyDescription)} \hfill \textit{\ACTLocation}}\\
\textit{\ACTJobTitle} \hfill \textit{\ACTJobStart\,--\,\ACTJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \ACTTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \ACTFiscal
    \item \ACTSolutions
    \item \ACTSLA
\end{itemize}

\item \parbox{\textwidth}{\textbf{\HSBCName} {\color{secondary}\small(\HSBCCompanyDescription)} \hfill \textit{\HSBCLocation}}\\
\textit{\HSBCJobTitle} \hfill \textit{\HSBCJobStart\,--\,\HSBCJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \HSBCTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \HSBCShort
\end{itemize}

\item \parbox{\textwidth}{\textbf{\LloydsName} {\color{secondary}\small(\LloydsCompanyDescription)} \hfill \textit{\LloydsLocation}}\\
\textit{\LloydsJobTitle} \hfill \textit{\LloydsJobStart\,--\,\LloydsJobEnd}

\vspace{0.1cm}
{\color{secondary}Technologies:} \LloydsTechnology
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,topsep=-6pt,parsep=0pt,partopsep=0pt,itemsep=0pt]
    \item \LloydsProjects
    \item \LloydsFiscal
\end{itemize}

\begin{samepage}
\needspace{13\baselineskip}
\section{Education}
\begin{itemize}[leftmargin=0pt,label={},itemsep=2em]
\item \parbox{\textwidth}{\textbf{\MasterUniversity} \hfill \textit{\MasterLocation}}\\
\textit{\MasterCourse} \hfill \textit{Graduated \MasterGraduation}\\[4pt]
\begin{itemize}[label={--},leftmargin=*,topsep=0pt,parsep=0pt,partopsep=0pt]
    \item {\color{secondary}Thesis:} \textit{\MastersThesisTitle}\\
    \MastersThesisDescription
    \item {\color{secondary}Courses:} \MastersCourseList
    \item {\color{secondary}Pre-Masters:} \PreMastersCourseList
    \item {\color{secondary}Languages Used:} \MastersLanguages
\end{itemize}
{Note: \textit{\MastersNotableAchievement}}

\item \parbox{\textwidth}{\textbf{\BatchelorsUniversity} \hfill \textit{\BatchelorsLocation}}\\
\textit{\BatchelorsCourse} \hfill \textit{Graduated \BatchelorsGraduation}
\parbox{\textwidth}{\textbf{\HEAOHogeschool} \hfill \textit{\HEAOsLocation}}\\
\textit{\HEAOCourse} \hfill \textit{Graduated \BatchelorsGraduation}
\end{itemize}
\end{samepage}


\section{Certifications}
\begin{itemize}[leftmargin=*]
\item \CertMicrosoft
\item \CertDutch
\item \OtherDrivingLicense
\end{itemize}

\section{Languages}
\begin{itemize}[leftmargin=*]
\item \textbf{English:} \LangEnglish
\item \textbf{Dutch:} \LangDutch
\end{itemize}

\end{itemize}

\end{document}